...
namespace Discover.Colocation
{
    public class ColocationDriverNetObj : NetworkBehaviour, INetworkRunnerCallbacks
    {
        ...
        private async UniTask SetupForColocation()
        {
            ...
            if (HasStateAuthority)
            {
                DebugPanel.Instance.UpdateMessage("Has State Authority");
                m_colocationLauncher.CreateColocatedSpace();
            }
            else
            {
                DebugPanel.Instance.UpdateMessage("No State Authority");
                // Don't try to colocate if we join remotely
                if (SkipColocation)
                {
                    OnColocationCompletedCallback?.Invoke(false);
                    DebugPanel.Instance.UpdateMessage("Skip Colocation is True");
                }
                else
                {
                    DebugPanel.Instance.UpdateMessage("Skip Colocation is False");
        ...
        private void OnAfterColocationReady()
        {
            ...
            DebugPanel.Instance.UpdateMessage("Colocation is ready");
        ...
        private void OnColocationFailed()
        {
            ...
            DebugPanel.Instance.UpdateMessage("Colocation failed");
            OnColocationCompletedCallback?.Invoke(false);
        }
